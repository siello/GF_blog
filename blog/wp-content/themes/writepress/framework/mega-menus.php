...
add_action( 'wp_nav_menu_item_custom_fields', 'writepress_add_megamenu_fields', 10, 4 );
function writepress_add_megamenu_fields( $item_id, $item, $depth, $args ) { ?>
    ...
    <div class="zolo-mega-menu-options">
        <p class="field-megamenu-status description description-wide">
            <label for="edit-menu-item-megamenu-status-<?php echo $item_id; ?>">
                ...
                <strong><?php _e( 'Enable Mega Menu', 'writepress' ); ?></strong>
            </label>
        </p>
        ...
        <p class="field-megamenu-columns description description-wide">
            <label for="edit-menu-item-megamenu-columns-<?php echo $item_id; ?>">
                ...
                <select id="edit-menu-item-megamenu-columns-<?php echo $item_id; ?>" class="widefat code edit-menu-item-megamenu-columns" name="menu-item-zolo-megamenu-columns[<?php echo $item_id; ?>]">
                    <option value="auto" <?php selected( $item->zolo_megamenu_columns, 'auto' ); ?>><?php _e( 'Auto', 'writepress' ); ?></option>
        ...
        <p class="field-megamenu-columnwidth description description-wide" style="display:none!important;">
			<label for="edit-menu-item-megamenu-columnwidth-<?php echo $item_id; ?>">
				<?php _e( 'Mega Menu Column Width (in percentage, ex: 30%)', 'writepress' ); ?>
        ...
        <p class="field-megamenu-title description description-wide">
            <label for="edit-menu-item-megamenu-title-<?php echo $item_id; ?>">
                ...
                <?php _e( 'Disable Mega Menu Column Title', 'writepress' ); ?>
            </label>
        </p>
        <p class="field-megamenu-widgetarea description description-wide">
            <label for="edit-menu-item-megamenu-widgetarea-<?php echo $item_id; ?>">
                <?php _e( 'Mega Menu Widget Area', 'writepress' ); ?>
                <select id="edit-menu-item-megamenu-widgetarea-<?php echo $item_id; ?>" class="widefat code edit-menu-item-megamenu-widgetarea" name="menu-item-zolo-megamenu-widgetarea[<?php echo $item_id; ?>]">
                    <option value="0"><?php _e( 'Select Widget Area', 'writepress' ); ?></option>
                    <?php
        ...
        <p class="field-megamenu-icon description description-wide">
            <label for="edit-menu-item-megamenu-icon-<?php echo $item_id; ?>">
                <?php _e( 'Mega Menu Icon (use full font awesome name)', 'writepress' ); ?>
        ...
        <a href="#" id="zolo-media-upload-<?php echo $item_id; ?>" class="zolo-open-media button button-primary zolo-megamenu-upload-thumbnail"><?php _e( 'Set Thumbnail', 'writepress' ); ?></a>
        <p class="field-megamenu-thumbnail description description-wide">
            <label for="edit-menu-item-megamenu-thumbnail-<?php echo $item_id; ?>">
                ...
                <img src="<?php echo $item->zolo_megamenu_thumbnail; ?>" id="zolo-media-img-<?php echo $item_id; ?>" alt="<?php echo esc_attr( get_bloginfo( 'name' ) ); ?>" class="zolo-megamenu-thumbnail-image" style="<?php echo ( trim( $item->zolo_megamenu_thumbnail ) ) ? 'display: inline;' : '';?>" />
                <a href="#" id="zolo-media-remove-<?php echo $item_id; ?>" class="remove-zolo-megamenu-thumbnail" style="<?php echo ( trim( $item->zolo_megamenu_thumbnail ) ) ? 'display: inline;' : '';?>"><?php _e( 'Remove Image', 'writepress' ); ?></a>
            </label>
        </p>
...
if( ! class_exists( 'ZOLOCoreFrontendWalker' ) ) {
	class ZOLOCoreFrontendWalker extends Walker_Nav_Menu {
		...
		private function set_megamenu_max_width() {
			...
			$default_site_width = isset($writepress_data['site_width']['width']) ? $writepress_data['site_width']['width'] : '1280px';
			$default_megamenu_max_width = isset($writepress_data['megamenu_max_width']['width']) ? $writepress_data['megamenu_max_width']['width'] : '1280px';
			// set overall width of megamenu
			$site_width = (int) str_replace( 'px', '', $default_site_width);
			$megamenu_max_width = (int) str_replace( 'px', '', $default_megamenu_max_width);
			$megmanu_width = 0;

			if( strpos( $default_site_width, 'px' ) !== false ) {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$dropdown_indicator = isset($writepress_data['dropdown_indicator']) ? $writepress_data['dropdown_indicator'] : 'dropdown_indicator_hide';
			...
			if( $depth === 1 && $this->menu_megamenu_status == "enabled" ) {
				...
				if( !(
					...
					if ( ! empty( $this->menu_megamenu_thumbnail ) ) {
						$title_enhance = '<span class="zolo-megamenu-icon"><img src="' . $this->menu_megamenu_thumbnail . '" alt="'.esc_attr( get_bloginfo( 'name' ) ).'"></span>';
				...
				if ( ! empty( $this->menu_megamenu_thumbnail ) && $this->menu_megamenu_status == "enabled" ) {
					$item_output .= '<a ' . $attributes . '><span class="zolo-megamenu-icon"><img src="' . $this->menu_megamenu_thumbnail . '" alt="'.esc_attr( get_bloginfo( 'name' ) ).'"></span>';
				} elseif( ! empty( $this->menu_megamenu_icon ) && $this->menu_megamenu_status == "enabled" ) {
					$item_output .= '<a ' . $attributes . '><span class="zolo-megamenu-icon text-menu-icon"><i class="fa glyphicon ' . $this->menu_megamenu_icon . '"></i></span>';
				...
				if( $depth === 0 && $args->has_children && $dropdown_indicator == 'dropdown_indicator_show' ) {
		...
		function end_el( &$output, $item, $depth = 0, $args = array() ) {
			...
			if ( $header_layout == 'v8' && $this->middle_menu_break_point == $this->no_of_top_level_items_displayed && '0' === $item->menu_item_parent ) {
				ob_start();
				?>
                <?php
				// Header Logo
...
if( ! class_exists( 'ZOLOcodeCoreMegaMenus' ) ) {
    ...
    class ZOLOcodeCoreMegaMenus extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			global $_wp_nav_menu_max_depth, $wp_registered_sidebars , $writepress_data;
			...
			if ( ! empty( $item->_invalid ) ) {
				...
				$title = sprintf( __( '%s (Invalid)', 'writepress'), $item->title );
			} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
				$classes[] = 'pending';
				/* translators: %s: title of menu item in draft status */
				$title = sprintf( __('%s (Pending)', 'writepress'), $item->title );
			}
			...
			<li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes ); ?>">
				<dl class="menu-item-bar">
					<dt class="menu-item-handle">
						<span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo $submenu_text; ?>><?php _e( 'sub item', 'writepress' ); ?></span></span>
						<span class="item-controls">
							...
							<span class="item-order hide-if-js">
								...
								?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'writepress'); ?>">&#8593;</abbr></a>
								|
								<a href="<?php
									echo wp_nonce_url(
										add_query_arg(
								...
								?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'writepress'); ?>">&#8595;</abbr></a>
							</span>
							<a class="item-edit" id="edit-<?php echo $item_id; ?>" title="<?php esc_attr_e('Edit Menu Item', 'writepress'); ?>" href="<?php
								echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
							?>"><?php _e( 'Edit Menu Item', 'writepress' ); ?></a>
						</span>
					</dt>
				</dl>
				...
				<div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
					...
					<p class="description description-thin">
						<label for="edit-menu-item-title-<?php echo $item_id; ?>">
							<?php _e( 'Navigation Label' , 'writepress'); ?><br />
							<input type="text" id="edit-menu-item-title-<?php echo $item_id; ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
						</label>
					</p>
					<p class="description description-thin">
						<label for="edit-menu-item-attr-title-<?php echo $item_id; ?>">
							<?php _e( 'Title Attribute' , 'writepress'); ?><br />
					...
					<p class="field-link-target description">
						<label for="edit-menu-item-target-<?php echo $item_id; ?>">
							...
							<?php _e( 'Open link in a new window/tab' , 'writepress'); ?>
						</label>
					</p>
					<p class="field-css-classes description description-thin">
						<label for="edit-menu-item-classes-<?php echo $item_id; ?>">
							<?php _e( 'CSS Classes (optional)' , 'writepress'); ?><br />
							<input type="text" id="edit-menu-item-classes-<?php echo $item_id; ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo $item_id; ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
						</label>
					</p>
					<p class="field-xfn description description-thin">
						<label for="edit-menu-item-xfn-<?php echo $item_id; ?>">
							<?php _e( 'Link Relationship (XFN)' , 'writepress'); ?><br />
							<input type="text" id="edit-menu-item-xfn-<?php echo $item_id; ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
						</label>
					</p>
					<p class="field-description description description-wide">
						<label for="edit-menu-item-description-<?php echo $item_id; ?>">
							<?php _e( 'Description' , 'writepress'); ?><br />
							<textarea id="edit-menu-item-description-<?php echo $item_id; ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo $item_id; ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
							<span class="description"><?php _e('The description will be displayed in the menu if the current theme supports it.', 'writepress'); ?></span>
						</label>
					</p>
					...
					<p class="field-move hide-if-no-js description description-wide">
						<label>
							<span><?php _e( 'Move' , 'writepress'); ?></span>
							<a href="#" class="menus-move-up"><?php _e( 'Up one', 'writepress' ); ?></a>
							<a href="#" class="menus-move-down"><?php _e( 'Down one' , 'writepress'); ?></a>
							<a href="#" class="menus-move-left"></a>
							<a href="#" class="menus-move-right"></a>
							<a href="#" class="menus-move-top"><?php _e( 'To the top' , 'writepress'); ?></a>
						</label>
					</p>
					...
					<div class="menu-item-actions description-wide submitbox">
						<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
							<p class="link-to-original">
								<?php printf( __('Original: %s', 'writepress'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
							</p>
						<?php endif; ?>
						...
						); ?>"><?php _e( 'Remove' , 'writepress'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo $item_id; ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
							?>#menu-item-settings-<?php echo $item_id; ?>"><?php _e('Cancel', 'writepress'); ?></a>
					</div>
