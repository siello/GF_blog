...
class sidebar_generator {
	...
	public static function add_meta_boxes() {
		...
		foreach ( $post_types as $post_type ) {
			add_meta_box( 'sbg_box', esc_html__( 'Sidebar', 'writepress-core' ), array( 'sidebar_generator', 'edit_form' ), $post_type, 'side' );
		}
	}
	...
	public static function add_sidebar(){
		...
		if ( isset( $sidebars[ $id ] ) ) {
			die("alert('" . esc_html__( 'Widget Section already exists, please use a different name.', 'writepress-core' ) . "')" );
		}
	...
	public static function remove_sidebar(){
		...
		$no_widget_text = esc_html__( 'No Widget Sections defined.', 'writepress-core' );

		$id = sidebar_generator::name_to_class( $name );
		if ( ! isset( $sidebars[ $id ] ) ) {
			die( 'alert("' . esc_html__( 'Sidebar does not exist.', 'writepress-core' ) . '")' );
		}
	...
	public static function admin_menu() {
		add_theme_page( esc_html__( 'Widget Sections', 'writepress-core' ), esc_html__( 'Widget Sections', 'writepress-core' ), 'manage_options', 'multiple_sidebars', array( 'sidebar_generator', 'admin_page' ) );
	}

	public static function admin_page() { ?>
		<script>
		function remove_sidebar_link(name,num){
			answer = confirm( "<?php esc_html_e( 'Are you sure you want to remove', 'writepress-core' ); ?> " + name + "?\n<?php esc_html_e( 'This will remove any widgets you have assigned to this widget section.', 'writepress-core' ); ?>" );
				if ( answer ) {
					//alert('AJAX REMOVE');
			...
			function add_sidebar_link(){
				var sidebar_name = prompt( "<?php esc_html_e( 'Widget Section Name', 'writepress-core' ); ?>","" );
				//alert(sidebar_name);
		...
		<div class="postbox" style="max-width:1719px;">
			<h2 class="hndle ui-sortable-handle" style="padding: 15px 12px; margin: 0;">
				<span><?php esc_html_e( 'Widget Sections', 'writepress-core' ); ?></span>
			</h2>
			<div class="inside" style="margin-bottom: 0;">
				<table class="widefat page" id="sbg_table">
					<tr>
						<th><?php esc_html_e( 'Widget Section Name', 'writepress-core' ); ?></th>
						<th><?php esc_html_e( 'CSS Class', 'writepress-core' ); ?></th>
						<th><?php esc_html_e( 'Remove', 'writepress-core' ); ?></th>
					</tr>
					<?php
					$sidebars = sidebar_generator::get_sidebars();
					?>
					<?php if ( is_array( $sidebars ) && ! empty( $sidebars ) ) : ?>
						...
						<?php foreach ( $sidebars as $sidebar ) : ?>
							...
							<tr class="<?php echo esc_attr($alt); ?>">
								...
								<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo esc_attr($sidebar); ?>',<?php echo $cnt + 1; ?>);" title="<?php esc_html_e( 'Remove this Widget Section', 'writepress-core' ); ?>"><?php esc_attr_e( 'remove', 'writepress-core' ); ?></a></td>
							</tr>
							<?php $cnt++; ?>
						<?php endforeach; ?>
					<?php else : ?>
						<tr id="no-widget-sections">
							<td colspan="3"><?php esc_html_e( 'No Widget Sections defined.', 'writepress-core' ); ?></td>
						</tr>
					<?php endif; ?>
				</table>
				<p class="add_sidebar"><a href="javascript:void(0);" onclick="return add_sidebar_link()" title="<?php _e( 'Add New Widget Section', 'writepress-core' ); ?>" class="button button-primary"><?php _e( 'Add New Widget Section', 'writepress-core' ); ?></a></p>
			</div>
		</div>
		<?php
	}
	...
	*/
	...
	public static function edit_form() {
		...
		<div class="zolo_metabox_field">
			...
			<div class="zolo_desc">
				<label><?php esc_html_e( 'Select Sidebar 1:', 'writepress-core' ); ?></label>
				<p><?php esc_html_e( 'Select sidebar 1 that will display on this page. Choose "No Sidebar" for full width.', 'writepress-core' ); ?></p>
			</div>
			<div class="zolo_field">
				...
				<?php for ( $i = 0; $i < 1; $i++ ) : ?>
					...
					<select name="sidebar_generator[<?php echo $i; ?>]" style="display: none;">
						<option value="0"<?php echo ( '' == $selected_sidebar[ $i ] ) ? ' selected' : ''; ?>><?php esc_html__( 'WP Default Sidebar', 'writepress-core' ); ?></option>
						<?php $sidebars = $wp_registered_sidebars; // sidebar_generator::get_sidebars(); ?>
						<?php if ( is_array( $sidebars ) && ! empty( $sidebars ) ) : ?>
							<?php foreach ( $sidebars as $sidebar ) : ?>
								<?php if ( $selected_sidebar[ $i ] == $sidebar['name'] ) : ?>
									<?php if ( 'Left Sidebar Widget Area' == $sidebar['name'] || esc_html__( 'Left Sidebar Widget Area', 'writepress-core' ) == $sidebar['name'] ) : ?>
										<option value="<?php echo esc_attr( $sidebar['name'] ); ?>" selected><?php esc_html_e( 'Default Sidebar', 'writepress-core' ); ?></option>
									<?php else : ?>
										<option value="<?php echo esc_attr( $sidebar['name'] ); ?>" selected><?php esc_html( $sidebar['name'] ); ?></option>
									<?php endif; ?>
								<?php else : ?>
									<?php if ( 'Left Sidebar Widget Area' == $sidebar['name'] || esc_html__( 'Left Sidebar Widget Area', 'writepress-core' ) == $sidebar['name'] ) : ?>
										<option value="<?php echo esc_attr( $sidebar['name'] ); ?>"><?php esc_html_e( 'Default Sidebar', 'writepress-core' ); ?></option>
									<?php else : ?>
					...
					<select name="sidebar_generator_replacement[<?php echo $i; ?>]">
						<option value="" <?php echo ( '' == $selected_sidebar_replacement[ $i ] && 'post' != $screen->post_type ) ? ' selected' : ''; ?>><?php esc_html_e( 'No Sidebar', 'writepress-core' ); ?></option>
						...
						<?php if ( is_array( $sidebar_replacements ) && ! empty( $sidebar_replacements ) ) : ?>
							<?php foreach ( $sidebar_replacements as $sidebar ) : ?>
								<?php if ( '0' == $selected_sidebar_replacement[ $i ] ) : ?>
									<?php $selected_sidebar_replacement[ $i ] = esc_html__( 'Left Sidebar Widget Area', 'writepress-core' ); ?>
								<?php endif; ?>
								...
								<?php else : ?>
									<?php if ( 'Left Sidebar Widget Area' == $sidebar['name'] || esc_html__( 'Left Sidebar Widget Area', 'writepress-core' ) == $sidebar['name'] ) : ?>
										...
										<option value="<?php echo esc_attr( $sidebar['name'] ); ?>"<?php echo $selected; ?>><?php esc_html_e( 'Default Sidebar', 'writepress-core' ); ?></option>
									<?php else : ?>
										<option value="<?php echo esc_attr( $sidebar['name'] ); ?>"/><?php esc_html( $sidebar['name'] ); ?></option>
									<?php endif; ?>
								<?php endif; ?>
			...
			if ( is_array( $selected_sidebar ) && ! empty( $selected_sidebar ) ) {
				...
				for ( $i = 0; $i < $sizeof_selected_sidebar; $i++ ) {
					if ( '0' == $name && '0' == $selected_sidebar[ $i ] && '0' == $selected_sidebar_replacement[ $i ] ) {
						...
						$did_sidebar = true;
						break;
					} elseif ( '0' == $name && '0' == $selected_sidebar[ $i ] || 'Left Sidebar Widget Area' == $selected_sidebar[ $i ] || esc_html__( 'Left Sidebar Widget Area', 'writepress-core' ) == $selected_sidebar[ $i ] ) {
